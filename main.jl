...
instance, warehouse, depot_distance, depot_id, cooc, skus, sparse_matrix, U, V, S_hat, Distance_matrix, P, W, S_hat_hat, allocations = allocation_main(do_allocation, do_mensage_passing, data_id, data_names, n_aisles, n_shelves, capacity, sigma, v_tipico, p_frequency, p_volume, p_quantity, volumetricModule, warehouse_capacity, warehouse_filling_rate, seed, candidate_fraction, als_k, als_factor, distance_factor, message_passing_factor, max_variety)

results = route_orders(
    instances,
    allocations,
    skus,
    picker_capacity,
...
all_pickers = reduce(vcat, results)

sku_volume = [sku.volume for sku in skus]

merged_pickers = clarke_wright(
    all_pickers,
    distance_matrix,
    sku_volume,
...
print_pickers(merged_pickers, do_allocation, do_mensage_passing, data_name)
