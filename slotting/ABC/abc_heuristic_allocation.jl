# scr/Solotting/ABC/abc_heuristic_allocation.jl

function abc_allocation_heuristic(
    skus::Vector{SKU},
    classes::Vector{Char},
    ...
    @assert length(classes) == n_skus "classes deve ter o mesmo tamanho de skus"
    ...
    variety_count = zeros(Int, n_shelves)
    ...
    for sku_id in order
        ...
        while remaining > 0
            ...
            for shelf in 1:n_shelves
                ...
                already_present = allocation[shelf, sku_id] > 0

                if !already_present && variety_count[shelf] >= max_variety
                    continue
                end
            ...
            if best_shelf == 0
                error(
                    "Nao ha capacidade suficiente (ou limite de max_variety " *
                    "atingido) para alocar o SKU $sku_id"
                )
            end

            if allocation[best_shelf, sku_id] == 0
                variety_count[best_shelf] += 1
            end
